...
void prepare(int width, int height)
{
    ...
    state.textureFilter = Material::Linear_MipNearest;
    state.textureAddress = Material::Wrap;
...
bool popState()
{
    ...
    g_pImmediateMode->setTextureFilter((Material::TextureFilter)s.textureFilter);
    g_pImmediateMode->setTextureAddress((Material::AddressMode)s.textureAddress);
    return true;
}
...
void translate(const Vec3& v)
{
    ...
    t.makeIdentity();
    t.pos = v;
    top().transform = top().transform * Matrix4x4(t);
}
// 0x004bcbe0
void scale(const Vec3& v)
{
    flush();
    Matrix4x3 t(Matrix3x3(Vec3(v.x, 0, 0), Vec3(0, v.y, 0), Vec3(0, 0, v.z)), Vec3(0, 0, 0));
    top().transform = top().transform * Matrix4x4(t);
}
// 0x004bcc50
...
{
    flush();
    Matrix4x3 t(Matrix3x3::createRotation(axis, angle), Vec3(0, 0, 0));
    top().transform = top().transform * Matrix4x4(t);
}
...
{
    flush();
    top().textureFilter = filter;
    g_pImmediateMode->setTextureFilter(filter);
}
// 0x004bceb0
void setTextureAddressMode(Material::AddressMode mode)
{
    flush();
    top().textureAddress = mode;
    g_pImmediateMode->setTextureAddress(mode);
}

// 0x004bd1d0
void beginShape(int type, RenderableTexture* texture, bool threeDee)
{
    select(type, texture, threeDee);
}
// 0x004be350
...
{
    g_pCurrentVertex = allocVertex(1);
    g_pCurrentVertex->pos.set(x, y, z);
}
// 0x081035e0
...
// 0x004be5d0: breaks at spaces once a word would cross the width; a word longer than the
// width is broken where it overflows.
void drawParagraph(const char* text, int x, int y, int width, Font* font, const Color& color,
                   int* maxLineWidth, int* endY)
{
    ...
    for (;;)
    {
        ...
        if (ch == 0)
        {
            drawText(lineStart, Vec2((float)x, (float)y), font, color, (int)(p - lineStart));
            if (maxLineWidth)
            {
                int w = font->getWidth(lineStart, (int)(p - lineStart));
                if (w > *maxLineWidth)
                    *maxLineWidth = w;
            }
            if (endY)
                *endY = font->getLineHeight() - startY + y;
            return;
        }
        if (ch == '\n')
        {
        ...
        if (lineWidth + glyph->width > width)
        {
            if (lastSpace)
            {
                int len = (int)(lastSpace - lineStart);
                drawText(lineStart, Vec2((float)x, (float)y), font, color, len);
                if (maxLineWidth)
                {
                    int w = font->getWidth(lineStart, len);
                    if (w > *maxLineWidth)
                        *maxLineWidth = w;
                }
                p = lastSpace + 1;
