...
#   grimrock2             <- stripped build-release/grimrock2
...
cmake --build "$ROOT/build-release" --target grimrock2 -j"$(nproc)"

strip -o "$GAME/grimrock2" "$ROOT/build-release/grimrock2"
...
mkdir -p "$GAME/lib64"
ldd "$ROOT/build-release/grimrock2" | awk '/libminizip|libGLEW|libSDL3/ { print $1, $3 }' |
