...
strings1 = {a: set(STRING.findall(c)) for a, c in code1.items()}

# ---- LoG2 -------------------------------------------------------------------------
code2 = {p.stem: p.read_text() for p in L2.glob('*.c')}
strings2 = {a: set(STRING.findall(c)) for a, c in code2.items()}
...
for mod, classes in bindings.items():
    for c in classes:
        for method, addr in c['methods']:
            ...
            result[addr] = {'name': '%s_%s' % (cls.replace('.', '_'), method), 'how': 'binding', 'score': 1.0}
...
for a2, s2 in strings2.items():
    ...
    if score >= 4.0 and score > second * 1.5 and len(shared) >= 1:
...
while changed and rounds < 5:
    ...
    for a2, info in list(result.items()):
        ...
        if a1 is None or a2 not in code2 or a1 not in code1:
            continue
        c1, c2 = calls1(a1), calls2(a2)
        if len(c1) != len(c2) or not c1:
            continue
        for x1, x2 in zip(c1, c2):
            if x1 and x2 and x2 not in result and x1 in names1 and x2 in code2:
                result[x2] = {'name': names1[x1], 'how': 'calls', 'score': 0.5, 'log1': x1}
                changed = True
